    ...
    <div style="min-height: 100vh;height: 100%;display: flex;justify-content: center;align-items: center;">
        <v-card class="mx-auto" style="width: 100%;max-width: 400px;max-height: 100%;" variant="flat" rounded="lg">
            ...
            <v-form v-model="form" @submit.prevent="onSubmit">
                <v-text-field v-model="fullname" :readonly="loading" :rules="[required]" label="Họ và tên"
...
const onSubmit = async () => {
    ...
    const res = await register({ email: emailValue, password: passwordValue, fullname: fullnameValue });
    if (res.success) {
        setTimeout(() => {
            loading.value = false;
            router.push('/login');
